...
test.describe('Settings provider and role configuration', () => {
  test('exposes independently editable Codex and Copilot providers and roles', async ({ page, request }) => {
    ...
    const settings = await initial.json() as { providers: Array<{ id: string; cliCommand: string }>; roles: Array<{ id: string }> };
    ...
    await page.getByLabel('Model').fill('gpt-5.2-codex');
    ...
    const snapshot = await resolved.json() as { binding: { providerId: string; model: string; thinking: string } };
