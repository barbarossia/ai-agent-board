      ...
      <motion.div role="dialog" aria-modal="true" aria-label="Settings" initial={{ opacity: 0, scale: 0.95, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95, y: 20 }} className="fixed left-1/2 top-1/2 z-50 flex max-h-[90vh] w-[calc(100%-2rem)] max-w-3xl -translate-x-1/2 -translate-y-1/2 flex-col overflow-hidden rounded-xl border border-border bg-card shadow-2xl">
        ...
        {!settings ? <div className="p-6 text-sm text-muted-foreground">Loading settings…</div> : tab === 'providers' ? (
          <form onSubmit={saveProvider} className="grid min-h-0 flex-1 gap-5 overflow-y-auto p-6 md:grid-cols-[12rem_1fr]">
            ...
            {provider && <div className="space-y-4"><div className="flex items-center justify-between"><h3 className="font-medium">{provider.displayName}</h3><label className="flex items-center gap-2 text-sm"><input type="checkbox" checked={provider.enabled} onChange={event => setProvider({ ...provider, enabled: event.target.checked })} /> Enabled</label></div><div className={`rounded-lg border px-3 py-2 text-sm ${validation.get(provider.id)?.available ? 'border-emerald-500/30 bg-emerald-500/10 text-emerald-600 dark:text-emerald-400' : 'border-amber-500/30 bg-amber-500/10 text-amber-700 dark:text-amber-300'}`}>{validation.get(provider.id)?.available ? `Ready${validation.get(provider.id)?.version ? ` · ${validation.get(provider.id)?.version}` : ''}` : validation.get(provider.id)?.reason || 'CLI availability has not been confirmed'}</div><label className="block text-xs font-medium text-muted-foreground">CLI command<input value={provider.cliCommand} onChange={event => setProvider({ ...provider, cliCommand: event.target.value })} className="mt-1.5 w-full rounded-lg border border-border bg-background px-3 py-2 font-mono text-sm" /></label><label className="block text-xs font-medium text-muted-foreground">Command arguments (comma-separated)<input value={csv(provider.commandArgs)} onChange={event => setProvider({ ...provider, commandArgs: splitCsv(event.target.value) })} className="mt-1.5 w-full rounded-lg border border-border bg-background px-3 py-2 font-mono text-sm" /></label><label className="block text-xs font-medium text-muted-foreground">Available models (comma-separated)<input value={csv(provider.models)} onChange={event => { const models = splitCsv(event.target.value); setProvider({ ...provider, models, defaultModel: models.includes(provider.defaultModel ?? '') ? provider.defaultModel : models[0] }); }} className="mt-1.5 w-full rounded-lg border border-border bg-background px-3 py-2 text-sm" /></label><label className="block text-xs font-medium text-muted-foreground">Default model<select aria-label="Default model" value={provider.defaultModel ?? provider.models[0] ?? ''} onChange={event => setProvider({ ...provider, defaultModel: event.target.value })} disabled={provider.models.length === 0} className="mt-1.5 w-full rounded-lg border border-border bg-background px-3 py-2 text-sm">{provider.models.length === 0 ? <option value="">No models configured</option> : provider.models.map(model => <option key={model} value={model}>{model}</option>)}</select></label><div className="flex items-center gap-2"><button type="button" onClick={testModel} disabled={testing || !provider.enabled || provider.models.length === 0} className="rounded-lg border border-border px-3 py-2 text-xs font-medium hover:bg-accent disabled:opacity-50">{testing ? 'Testing…' : 'Test model availability'}</button>{testResult && <span className={`text-xs ${testResult.available ? 'text-emerald-600 dark:text-emerald-400' : 'text-red-500'}`}>{testResult.available ? `${testResult.message || 'Model is available'}${testResult.version ? ` · ${testResult.version}` : ''}` : testResult.error || testResult.message || 'Model is unavailable'}</span>}</div><label className="block text-xs font-medium text-muted-foreground">Capabilities (comma-separated)<input value={csv(provider.capabilities)} onChange={event => setProvider({ ...provider, capabilities: splitCsv(event.target.value) })} className="mt-1.5 w-full rounded-lg border border-border bg-background px-3 py-2 text-sm" /></label><div className="rounded-lg border border-border bg-background p-3"><label className="block text-xs font-medium text-muted-foreground">Clone root<input value={cloneRoot} onChange={event => setCloneRoot(event.target.value)} className="mt-1.5 w-full rounded-lg border border-border bg-background px-3 py-2 font-mono text-sm" /></label><button type="button" onClick={saveCloneRoot} disabled={submitting} className="mt-2 rounded-lg border border-border px-3 py-1.5 text-xs font-medium hover:bg-accent disabled:opacity-50">Save clone root</button></div><div className="flex justify-end"><button type="submit" disabled={submitting} className="rounded-lg bg-primary px-4 py-2 text-sm font-medium text-primary-foreground disabled:opacity-50">{submitting ? 'Saving…' : 'Save provider'}</button></div></div>}
          </form>
        ) : (
